    subroutine CalculaTemperaturas
...
!=================================================================================================================================================================================
    ...
    Temperatura(1,1) = (1.0/(-1. + (beta(1,1)/alfa(1,1)))) * ((-1./3.)*(4*TemperaturaOld(2,1) - TemperaturaOld(3,1)) + beta(1,1)/(3*alfa(1,1))*(4*TemperaturaOld(1,2) - TemperaturaOld(1,3))  ) 
...
!=================================================================================================================================================================================
	...
	Temperatura(i,1) = (1./3.)*(4*TemperaturaOld(i,2) -TemperaturaOld(i,3)) - (beta(i,1)/(3*gama(i,1))) *(TemperaturaOld(i+1,1) - Temperatura(i-1,1))
...
!=================================================================================================================================================================================
    ...
    Temperatura(1,j)=(1./( (3*Kc*alfa(1,j)/(2*Jacobiano(1,j)*SQRT(alfa(1,j))*h_int)) + 1 ))&
                      *( (Kc/(2*Jacobiano(1,j)*SQRT(alfa(1,j))*h_int))*(alfa(1,j)*(4*TemperaturaOld(2,j) - TemperaturaOld(3,j))&
                      - beta(1,j)*(TemperaturaOld(1,j+1) - Temperatura(1,j-1))) + T_int) 
...
!---------------------------------------------------------------------COM GAP------------------------------------------------------------------------------------------------------
    ...
    Temperatura(i,j) = (0.5/(alfa(i,j)+gama(i,j)))*( alfa(i,j)*(TemperaturaOld(i+1,j) + Temperatura(i-1,j))&
                 - 0.5*beta(i,j)*(TemperaturaOld(i+1,j+1)-TemperaturaOld(i-1,j+1)-TemperaturaOld(i+1,j-1)+Temperatura(i-1,j-1))&
                 + gama(i,j)*(TemperaturaOld(i,j+1) + Temperatura(i,j-1)))
    ...
    Temperatura(F1-1,j) =(1./( (-3*Kc*alfa(F1-1,j)*ro/(2*Jacobiano(F1-1,j)*SQRT(alfa(F1-1,j))*hg*rfi)) -1 ))&
                      *( (Kc*ro/(2*Jacobiano(F1-1,j)*SQRT(alfa(F1-1,j))*hg*rfi))*(alfa(F1-1,j)*(-4*Temperatura(F1-2,j) + Temperatura(F1-3,j))&
                      - beta(F1-1,j)*(TemperaturaOld(F1-1,j+1) - Temperatura(F1-1,j-1))) - TemperaturaOld(F1,j))
...
!----------------------------------------------------------------------COM GAP-------------------------------------------------------------------------
    ...
    Temperatura(F1,j)=(1./( (3*Kf*alfa(F1,j)*rfi/(2*Jacobiano(F1,j)*SQRT(alfa(F1,j))*hg*ro)) + 1 ))&
                      *( (Kf*rfi/(2*Jacobiano(F1,j)*SQRT(alfa(F1,j))*hg*ro))*(alfa(F1,j)*(4*TemperaturaOld(F1+1,j) - TemperaturaOld(F1+2,j))&
                      - beta(F1,j)*(TemperaturaOld(F1,j+1) - Temperatura(F1,j-1))) + Temperatura(F1-1,j))
        
    DO i=F1+1, F2-1
	
    Temperatura(i,j)=(0.5/(alfa(i,j)+gama(i,j)))*( alfa(i,j)*(TemperaturaOld(i+1,j) + Temperatura(i-1,j))&
                        - 0.5*beta(i,j)*(TemperaturaOld(i+1,j+1)-TemperaturaOld(i-1,j+1)-TemperaturaOld(i+1,j-1)+Temperatura(i-1,j-1))&
                        + gama(i,j)*(TemperaturaOld(i,j+1) + Temperatura(i,j-1))+ ( (Jacobiano(i,j))**2 * densidade_de_potencia/Kf))
    END DO
    ...
    Temperatura(F2,j)= (1./( (3*Kf*alfa(F2,j)*rfo/(2*Jacobiano(F2,j)*SQRT(alfa(F2,j))*hg*rci)) + 1 ))&
                      *( (-Kf*rfo/(2*Jacobiano(F2,j)*SQRT(alfa(F2,j))*hg*rci))*(alfa(F2,j)*(-4*Temperatura(F2-1,j) + Temperatura(F2-2,j))&
                      - beta(F2,j)*(TemperaturaOld(F2,j+1) - Temperatura(F2,j-1))) + TemperaturaOld(F2+1,j))
...
!-----------------------------------------------------------------------COM GAP---------------------------------------------------------------------------------
   IF (dg .NE. 0) THEN ! Sem gap, o primeiro ponto interno do GAP é o F2+1 (parede LC1), com GAP, o F2+1 é a parede do GAP
        ...
        Temperatura(F2+1,j) = (1./( (3*Kc*alfa(F2+1,j)*rci/(2*Jacobiano(F2+1,j)*SQRT(alfa(F2+1,j))*hg*rfo)) +1 ))&
                      *( (Kc*rci/(2*Jacobiano(F2+1,j)*SQRT(alfa(F2+1,j))*hg*rfo))*(alfa(F2+1,j)*(4*TemperaturaOld(F2+2,j) - TemperaturaOld(F2+3,j))&
                      - beta(F2+1,j)*(TemperaturaOld(F2+1,j+1) - Temperatura(F2+1,j-1))) + Temperatura(F2,j))
...
!===============================================================================================================================================================
    ...
    Temperatura(biconcavas,1) = (1.0/(1. + (beta(biconcavas,1)/alfa(biconcavas,1)))) * ((-1./3.)*(-4*Temperatura(biconcavas-1,1) + Temperatura(biconcavas-2,1)) &
        + beta(biconcavas,1)/(3*alfa(biconcavas,1))*(4*TemperaturaOld(biconcavas,2) - TemperaturaOld(biconcavas,3))  )
!===============================================================================================================================================================
    ...
    ! PAREDE LC3 - Parede CONVECTIVA externa da pastilha (CHECADO)
        ...
        Temperatura(biconcavas,j)=(1./( (3*Kc*alfa(biconcavas,j)/(2*Jacobiano(biconcavas,j)*SQRT(alfa(biconcavas,j))*h_out)) + 1 ))&
                      *( (-Kc/(2*Jacobiano(biconcavas,j)*SQRT(alfa(biconcavas,j))*h_out))*(alfa(biconcavas,j)*(-4*Temperatura(biconcavas-1,j) + Temperatura(biconcavas-2,j))&
                      - beta(biconcavas,j)*(TemperaturaOld(biconcavas,j+1) - Temperatura(biconcavas,j-1))) + T_out) 
!END DO
...
!===============================================================================================================================================================
    ...
    Temperatura(1,radiais) = (1.0/(-1. - (beta(1,radiais)/alfa(1,radiais)))) * ((-1./3.)*(4*TemperaturaOld(2,radiais) - TemperaturaOld(3,radiais))&
        + beta(1,radiais)/(3*alfa(1,radiais))*(-4*TemperaturaOld(1,radiais-1) + TemperaturaOld(1,radiais-2))  )
    ...
    Temperatura(biconcavas,radiais) = (1.0/(1. - (beta(biconcavas,radiais)/alfa(biconcavas,radiais)))) * ((-1./3.)*(-4*TemperaturaOld(biconcavas-1,radiais) + TemperaturaOld(biconcavas-2,radiais)) &
        + beta(biconcavas,radiais)/(3*alfa(biconcavas,radiais))*(-4*TemperaturaOld(biconcavas,radiais-1) + TemperaturaOld(biconcavas,radiais-2))  ) 
!===============================================================================================================================================================
...
DO i=2,biconcavas-1 !Considera o corte da simetria superior e inferior: essa parede reflexiva é calculada de uma vez, sem repartir em CLAD, GAP e FUEL.
    ...
    Temperatura(i,radiais) =(-1./3.)*(-4*TemperaturaOld(i,radiais-1) + Temperatura(i,radiais-2)) + (beta(i,radiais)/(3*gama(i,radiais)))*(Temperatura(i+1,radiais) - Temperatura(i-1,radiais))
...
END DO  
   ...
   END SUBROUTINE CalculaTemperaturas
