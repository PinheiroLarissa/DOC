   SUBROUTINE Cassini_geo
    ...
    !*********************************************************************************************************************************************************************************************************************   
    ! Programador: Larissa
    ! Última revisão: 14/07/2019
    !
    !Descrição da subortina Cassini_geo:
    !Módulo de definição geral, contendo: malha em x, y, coeficientes, número de bicôncavas e retas radiais
    ...
    INTEGER      :: i, j, g, indice_bic                         ! Contadores, i -->> linhas, u da parametrização, j -->> colunas, v da parametrização, g --> contador que corre entre bicôncavas nas retas radiais.
    INTEGER      :: k, q, p, col                                ! Contadores utilizados nos loops de concatenamento das matrizes de malha
    REAL(KIND=DP):: a, b, ab                                    ! Parâmetros de Cassini, 'a', 'b' e 'ab'; 'di': distância de cada bic6oncava até a curva interna.
    REAL(KIND=DP):: u0                                          ! u = u0 é o 'u' escolhido para a parametrização com base nos a e b dados pelo usuário.
    REAL(KIND=DP), DIMENSION(0:SIZE(v)-1):: ro0                 ! Vetor utilizada na obtenção dos da curva interna através da parametrização, com u = u0.

    REAL(KIND=DP), DIMENSION(0:SIZE(v)-1,0:SIZE(u)-1):: rho     ! Matriz utilizada na obtenção dos contornos através da parametrização.
    REAL(KIND=DP), DIMENSION(0:SIZE(v)-1,0:SIZE(u)-1):: x_cas   ! Matriz de coordenadas x dos contornos dados pelas parametrização de Cassini.
    REAL(KIND=DP), DIMENSION(0:SIZE(v)-1,0:SIZE(u)-1):: y_cas   ! Matriz de coordenadas y dos contornos dados pelas parametrização de Cassini.
    ...
    OPEN(UNIT = 1, FILE = 'geometria.txt', STATUS = 'unknown')           ! Salva a geometra malhada para fornecer como entrada do método de Thompson
    OPEN(UNIT = 200, FILE = 'contornos_cassini.txt', STATUS = 'unknown') ! Salva todos os contornos para que se possa analisar a qualidade da parametrizção.

    ! Distância entre os focos (m)
    a  = 0.3151
    
    ! Razão a/b que caracteriza as curvas de Cassini (m)
    ab = 0.843
    
    ! A distância entre um ponto da curva até os focos é b^2 (m^2)
    b = a/ab
    
    !***********************************************************************************************************************************************************************************
    !--------------------------------------------------------- PARAMETRIZAÇÃO DAS CURVAS DE CASSINI: TODOS OS CONTORNOS ----------------------------------------------------------------
    !***********************************************************************************************************************************************************************************
    
    ! Criação do vetor u, que vai de 0 a PI em (SIZE(u) - 1) passos
    DO i = 0, SIZE(u) - 1  
    u(i) =  i*PI/(SIZE(u) - 1)
    ENDDO
    
    ! Criação do vetor v, que vai de 0 a PI em (SIZE(v) - 1) passos
    DO j = 0, SIZE(u) - 1 
    v(j) = j*PI/(SIZE(u) - 1)
    ENDDO
    
   ! Criação das matrizes x e y de Cassini, para tração todos os contornos com o a escolhido, i ->> linhas, u   j->> colunas, v
    DO i = 0, SIZE(u) - 1 
        DO j = 0, SIZE(u) - 1
...
100 FORMAT(51(f14.9),/)
    WRITE(200,*) '======================================================================================'
    WRITE(200,*) 'CURVAS GERADAS UTILIZANDO A PARAMETRIZAÇÃO DE MOON E SPENCER PARA AS CURVAS DE CASSINI'
    WRITE(200,*) '======================================================================================'
...
!***********************************************************************************************************************************************************************************
    ...
    ! Cálculo de u = u0 para usar na parametrização com base no a e no b escolhidos acima
    u0 = log( (a**2 + b**2)/a**2 - 1. )
    
    
    ! Criação dos vetores x e y da curva interna (que será extrudada com a espessura dada pelo SpaceClaim, conforme método da secante, pg 128 caderno) 
 ...
 !  WRITE(*,*) 'x2=['
  !  WRITE(*,10000) X_Malha_FUEL(0,:)
   ...
   ! WRITE(*,*) 'y2=['
    ...
    X_Malha_FUEL(num_bic-1, 0) = 0                                                  ! O primeiro x da bic. externa é zero. 
    Y_Malha_FUEL(num_bic-1, 0) = y_curva_interna(0) + df                            ! O primeiro y da bic. externa é apenas a soma do primeiro pt. da bic. interna + a espessura.
            
    X_Malha_FUEL(num_bic-1, SIZE(v) - 1) = x_curva_interna(SIZE(v) - 1) + df        ! O último x da bic. externa é apenas a soma do último y da bic. interna + a espessura.                        
    Y_Malha_FUEL(num_bic-1, SIZE(v) - 1) = 0                                        ! O último y da bic. externa é zero.
    ...
    !2) Para continuar escrevendo a curva no sentido horário, vou espelhar tanto x quanto y;
    ...
    X_Malha_CLAD1(0, 0) = 0                                              ! O primeiro x da bic. interna do CLAD1 é zero.
    Y_Malha_CLAD1(0, 0) = Y_malha_FUEL(0,0) - dc1                        ! O primeiro y da bic. interna do CLAD1 é apenas a subtração do primeiro pt. da bic. interna do FUEL - a espessura do CLAD.
            
    X_Malha_CLAD1(0, SIZE(v)-1) = Y_malha_FUEL(0,SIZE(v) - 1) - dc1      ! O último x da bic. interna do CLAD1 é apenas a subtração do último y da bic. interna do FUEL - a espessura do CLAD.
    Y_Malha_CLAD1(0, SIZE(v)-1) = 0                                      ! O último y da bic. interna do CLAD1 é zero. 
   ...
   !! X_Malha_CLAD2(0,:) = X_Malha_FUEL(num_bic-1,:) ! Toda a coluna 0 é prenchida pela bicôncava definida pela curva externa do FUEL (primeira bicôncava concêntrica do CLAD2)
    ...
    ! Não tenho a curva externa ainda, mas tenho as duas "quinas" externas, preciso deles porque uso o ponto da frente e o de trás das bicôncavas interna e externa para o cáculo do coeficiente angular da reta secante.
    X_Malha_CLAD2(C2-1, 0) = 0                                                 ! O primeiro x da bic. externa é zero.
    Y_Malha_CLAD2(C2-1, 0) = Y_Malha_FUEL(num_bic-1,0) + dc2                   ! O primeiro y da bic. externa é apenas a soma do primeiro pt. da bic. externa do FUEL + a espessura do CLAD..
            
    X_Malha_CLAD2(C2-1, SIZE(v)-1) = X_Malha_FUEL(num_bic-1,SIZE(v) - 1) + dc2  ! O último x da bic. externa é apenas a soma do último y da bic. externa do FUEL + a espessura.
    Y_Malha_CLAD2(C2-1, SIZE(v)-1) = 0                                          ! O último y da bic. externa é zero.
